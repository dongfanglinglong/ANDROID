...
class AlertAdapter extends BaseAdapter {
	...
	public AlertAdapter(Context context, String title, String[] items, String exit, String cancel) {
		if (items != null && items.length > 0) {
			for (int i = 0, lenghth = items.length; i < lenghth; i++) {
				itemList.add(items[i]);
			}
		}
		this.types = new int[itemList.size() + 3];
		this.context = context;
		if (title != null && !title.equals("")) {
			types[0] = TYPE_TITLE;
			this.isTitle = true;
			itemList.add(0, title);
		}

		if (exit != null && !exit.equals("")) {
			// this.isExit = true;
			types[itemList.size()] = TYPE_EXIT;
			itemList.add(exit);
		}

		if (cancel != null && !cancel.equals("")) {
			// this.isSpecial = true;
			types[itemList.size()] = TYPE_CANCEL;
			itemList.add(cancel);
		}
	}
